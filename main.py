...
from articles left join log on log.path=concat('/article/',articles.slug)
...
# select date_trunc('day',log.time) "day", count(*) as cnt from log where
# status='404 NOT FOUND' group by 1;
# select date_trunc('day',log.time) "day",
# 100.0*(sum(case when status='404 NOT FOUND'then 1 else 0 end))/count(time)
#  as percent from log group by 1;
query3 = '''
select to_char(day,'Month DD,YYYY') as day,percent as percentage
from (select date_trunc('day',log.time) "day",
100.0*(sum(case when status='404 NOT FOUND'then 1 else 0 end))/count(time)
 as percent from log group by 1) as foo where percent>1.0'''
c.execute(query3)
result3 = c.fetchall()
print("Solution for 3rd Question")
for title, views in result3:
    print("{} - {}".format(title, views))
print("\n")
